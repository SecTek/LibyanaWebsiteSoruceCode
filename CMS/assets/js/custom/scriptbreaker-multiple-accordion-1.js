...
(function($){
    ...
    accordionze: function(options) {
        ...
        //open active First level

  		$this.find("li.active").each(function () {
  		    $(this).parent().parents("ul").slideDown(opts.speed);
  		    $(this).parent().parents("ul").parent("li").find("h4:first").html(opts.openedSign);
  		});

  		$this.parent().find("li a.FirstLevel").click(function () {
  		    if ($(this).parent().parent().find("ul").size() != 0) {
  		        ...
  		        if ($(this).parent().parent().find("ul:first").is(":visible")) {
  		            $(this).parent().parent().find("ul:first").slideUp(opts.speed, function () {
  		                $(this).parent().parent("li").find("h4:first").delay(opts.speed).html(opts.closedSign);
  		            });


  		        } else {
  		            $(this).parent().parent().find("ul:first").slideDown(opts.speed, function () {
  		                $(this).parent().parent("li").find("h4:first").delay(opts.speed).html(opts.openedSign);
  		            });
  		        }
  		    }
  		});
