<%@ page title="" language="C#" masterpagefile="~/CMS/MasterPage.master" autoeventwireup="true" inherits="Roamings_View, App_Web_roamings_view.aspx.1f2a64c3" %>

<%@ Register TagPrefix="cc1" Assembly="Custom_Button" Namespace="Custom_Button" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
      <script>
          $(window).load(function () {
              $('#tblData_Roaming').DataTable({
                  stateSave: true
              });
              $('#tblData_Roaming2').DataTable({
                  stateSave: true
              });
          });
</script>
...
<asp:Content ID="Content3" ContentPlaceHolderID="cphBreadcrumb" runat="Server">
    <ul id="breadcrumb">
        ...
        <li><%= Resources.CMS_Resource.lblRoaming %></li>
...
<asp:Content ID="Content5" ContentPlaceHolderID="cphBodyContent" runat="Server">
    <div class="body-nest" id="footable" runat="server">
        <table id="tblData_Roaming" class="table-striped  footable metro-blue gvDataClass" width="100%">
            <thead>
                <tr>
                ...
                <asp:Repeater ID="rptData" runat="server">
                    ...
                    <ItemTemplate>
                        <tr>
                            ...
                            <td class="Hiddein_TD"><%# Eval("TITLE") %></td>
                            <td class="Hiddein_TD"><%# Eval("LIBYA") %></td>
                            <td class="Hiddein_TD"><%# Eval("AR_EUR_COUNTRIES") %></td>
                            <td class="Hiddein_TD"><%# Eval("REST_OF_WORD") %></td>
                            <td class="Hiddein_TD"><%# Eval("THURAIA") %></td>
                            <td class="Hiddein_TD"><%# Eval("SATELLITES") %></td>
                            <td class="Hiddein_TD"><%# Eval("RECEIVE_CALLING") %></td>
                            <td class="Hiddein_TD"><%# Eval("SMS") %></td>
                            <td class="Hiddein_TD"><%# Eval("COUNTRY_VALUE")%></td>
                            <td style="width: 15%">
                                <asp:HiddenField runat="server" ID="hfID_MAIN" Value='<%#DataBinder.Eval(Container,"DataItem.ID_MAIN") %>' />
                                <div id="Div1" runat="server" visible='<%# CheckFinalStatus(Convert.ToInt16(Eval("CHECKED_OUT_BY"))) %>'>
                                ...
                                <div id="Div2" runat="server" visible='<%# Convert.ToBoolean(Eval("IS_PUBLISHED")) %>'>
                                ...
                                <div id="Div3" runat="server" visible='<%# CheckFinalStatus(Convert.ToInt16(Eval("CHECKED_OUT_BY"))) %>'>
                                    <div id="Div4" runat="server" visible='<%# !(Convert.ToBoolean(Eval("IS_DELETED"))) %>'>
    ...
    <%-------------------------------------------------------------------------------------------------------------%>
    <%-------------------------------------------------------------------------------------------------------------%>
    <div id="Div5" class="div-auditing " runat="server">
    ...
    <div class="body-nest no-padding-bot">
        ...
        <table id="tblData_Roaming2" class="table-striped  footable metro-blue gvDataClass" width="100%">
                <thead>
                    <tr>
                        <th><%= Resources.CMS_Resource.lblTitle %></th>
                        ...
                        <ItemTemplate>
                            <tr>
                                ...
                                <td style="width: 15%">
                                    ...
                                    <div id="Div6" runat="server" visible='<%# CheckFinalStatus(Convert.ToInt16(Eval("ID_STATUS")),Convert.ToInt16(Eval("CREATED_BY"))) %>'>
                                    ...
                                    <div id="Div7" runat="server" visible='<%# CheckAuditDelete(Convert.ToInt16(Eval("ID_STATUS"))) %>'>
